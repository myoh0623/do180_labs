...
# DO180 Lab 6: 애플리케이션 스케일링 정리 스크립트
# 실습 후 리소스 정리
...
echo "  - Apollo 프로젝트 및 모든 리소스"
echo "  - jet 애플리케이션 deployment"
...
if oc get project apollo &>/dev/null; then
    echo -e "${GREEN}✓ Apollo 프로젝트 존재함${NC}"
    ...
    echo -e "${YELLOW}⚠ Apollo 프로젝트를 찾을 수 없습니다.${NC}"
    echo "이미 정리되었거나 생성되지 않았을 수 있습니다."
fi

echo ""

# 2. jet deployment 개별 삭제 (선택적)
echo -e "${YELLOW}[3/4] jet 애플리케이션 삭제 중...${NC}"

if oc get project apollo &>/dev/null; then
    oc project apollo &>/dev/null
    
    if oc get deployment jet &>/dev/null; then
        echo "  - jet deployment 삭제 중..."
        oc delete deployment jet --ignore-not-found=true
        echo -e "${GREEN}  ✓ jet deployment 삭제됨${NC}"
        
        # Pod 삭제 대기
        echo "  - jet pod 종료 대기 중..."
        oc wait --for=delete pod -l app=jet --timeout=60s 2>/dev/null || true
        echo -e "${GREEN}  ✓ jet pod 정리됨${NC}"
    else
        echo -e "${YELLOW}  ⚠ jet deployment를 찾을 수 없습니다.${NC}"
    fi
    
    # Service 삭제
    if oc get service jet &>/dev/null; then
        echo "  - jet service 삭제 중..."
        oc delete service jet --ignore-not-found=true
        echo -e "${GREEN}  ✓ jet service 삭제됨${NC}"
    fi
    
    # Route 삭제
    if oc get route jet &>/dev/null; then
        echo "  - jet route 삭제 중..."
        oc delete route jet --ignore-not-found=true
        echo -e "${GREEN}  ✓ jet route 삭제됨${NC}"
    fi
    
else
    echo -e "${YELLOW}  ⚠ Apollo 프로젝트가 없으므로 건너뜁니다.${NC}"
fi

echo ""

# 3. Apollo 프로젝트 삭제
echo -e "${YELLOW}[4/4] Apollo 프로젝트 삭제 중...${NC}"

if oc get project apollo &>/dev/null; then
    echo "  - Apollo 프로젝트 및 모든 리소스 삭제 중..."
    oc delete project apollo
    
    # 프로젝트 삭제 완료 대기
    echo "  - 프로젝트 삭제 완료 대기 중..."
    while oc get project apollo &>/dev/null; do
        echo -n "."
        sleep 2
    done
    echo ""
    echo -e "${GREEN}  ✓ Apollo 프로젝트 삭제 완료${NC}"
else
    echo -e "${YELLOW}  ⚠ Apollo 프로젝트가 이미 존재하지 않습니다.${NC}"
fi
